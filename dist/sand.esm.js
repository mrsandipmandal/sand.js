...
function renderComponent(rootEl, ctx) {
  ...
  walk(rootEl, (node) => {
    ...
    if (hasAttrEither(node, "modelable")) {
      try {
        sModelable && sModelable(node, ctx);
      } catch (e) {
    ...
    if (hasAttrEither(node, "transition")) {
      try {
        sTransition && sTransition(node, ctx);
      } catch (e) {
